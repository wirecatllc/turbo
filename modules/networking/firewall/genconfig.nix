...
let
  ...
  globalConfig = config.turbo.networking.firewall;
in
rec {
  ...
  rule = r:
    let
      ...
      beforeMacro = "${description}${filterExp}${extraFilterExp} ${actionExp};";
    in
    replaceStrings (map (macroName: "@${macroName}@") (attrNames globalConfig.macros)) (attrValues globalConfig.macros) beforeMacro;
  ...
  table = name: options:
    ...
    in
    if 0 == stringLength chains then "" else ''
      table ${name} {
  ...
  domain = name: options:
    ...
    in
    if 0 == stringLength tables then "" else ''
      domain ${name} {
      ${indent "  " tables}
      }
    '';
